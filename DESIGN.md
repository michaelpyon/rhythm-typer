...
4 states, all overlaying 1 fullscreen canvas:
...
4. **Paused.** Focus loss or Escape freezes the run and protects the combo. Resume resets the beat clock before play continues.

IA rule: 0 navigation, 0 pages, 0 extra modals beyond these 4 states. Depth comes from stats, not screens.
...
6. Pause screen (focus loss or Escape, 1-action resume)
...
- **Resolved in the release candidate:** `og:image` and `twitter:image` point to a 1200x630 PNG with image type and alt metadata. The legacy SVG stays as an undeployed design source.
